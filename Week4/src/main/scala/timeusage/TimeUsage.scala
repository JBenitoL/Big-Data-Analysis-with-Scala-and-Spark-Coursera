...
object TimeUsage extends TimeUsageInterface {
  ...
  /** @return An RDD Row compatible with the schema produced by `dfSchema`
    ...
    */
  def row(line: List[String]): Row = Row.fromSeq(Seq(line.head) ++ line.tail.map(_.toDouble))
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    val f1: List[String] = List("t01", "t03", "t11", "t1801", "t1803")
    val f2: List[String] = List("t05", "t1805")
    val f3: List[String] = List("t02", "t04", "t06", "t07", "t08", "t09", "t10", "t12", "t13",
      "t14", "t15", "t16", "t18")

    def containing(list: List[String], string: String): Boolean = {
      list match {
        case Nil => false
        case x :: xs => if (string.startsWith(x)) true else containing(xs, string)
      }

    }

    val prim: List[Column] = columnNames.filter(x => containing(f1, x)).map(col(_))
    val sec: List[Column] = columnNames.filter(x => containing(f2, x)).map(col(_))
    val third: List[Column] = columnNames.filter(x => !containing(f1, x) && !containing(f2, x)).
      filter(x => containing(f3, x)).map(col(_))
  ...
  /** @return SQL query equivalent to the transformation implemented in `timeUsageGrouped`
    ...
    */
  def timeUsageGroupedSqlQuery(viewName: String): String = "select working,sex,age,round(avg(primaryNeeds),1) as primaryNeeds,round(avg(work),1) as work,round(avg(other),1) as other from "+viewName+"" +
  ...
  def timeUsageSummaryTyped(timeUsageSummaryDf: DataFrame): Dataset[TimeUsageRow] =
    timeUsageSummaryDf.map(x => TimeUsageRow(
